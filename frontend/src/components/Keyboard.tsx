...
export default function Keyboard() {
  ...
  const getKeyStyle = (letter: string) => {
    ...
    return cn(
      "w-6 h-9 sm:size-12 text-sm sm:text-2xl rounded-md font-regular uppercase transition-colors",
      "hover:opacity-90 active:scale-95",
      {
        "border border-2 border-content-1": !state,
        "bg-accent-1 text-white": state === "correct",
        "bg-accent-2 text-white": state === "present",
            ...
            if (key === "ENTER") {
              return (
                <button
                  ...
                  className="size-9 sm:w-24 sm:h-12 flex justify-center items-center bg-content-1 text-content-2 rounded-md transition-colors"
                >
